/**
 * POST /api/admin/concierge — streaming internal ops concierge (admin only).
 */

import { errorResponse, AiError } from "@/lib/openrouter/errors";
import { chatStream, type ChatMessage } from "@/lib/openrouter/client";
import { ADMIN_CONCIERGE_SYSTEM } from "@/lib/openrouter/prompts";
...
export async function POST(req: Request) {
  try {
    ...
    const messages: ChatMessage[] = [
      { role: "system", content: ADMIN_CONCIERGE_SYSTEM },
      ...parsed.data.messages.map(
        (m) => ({ role: m.role, content: m.content }) as ChatMessage
      ),
    ];

    const stream = await chatStream({ task: "chat", messages, temperature: 0.55 });
